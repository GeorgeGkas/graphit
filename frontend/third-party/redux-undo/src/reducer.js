...
function insert(history, state, limit, group, filterStateProps) {
  ...
  const newPast =
    _latestUnfiltered != null ? [...pastSliced, _latestFiltered] : pastSliced
...
export default function undoable(reducer, rawConfig = {}) {
  ...
  const config = {
    ...
    filterStateProps:
      typeof rawConfig.filterStateProps !== 'function'
        ? _ => _
        : rawConfig.filterStateProps,
    ...
    switch (action.type) {
      ...
      default:
        ...
        const filtered =
          ...
          !config.filter(action, res, history)
