...
function runTests(
  label,
  { undoableConfig = {}, initialStoreState, testConfig } = {},
) {
  describe('Undoable: ' + label, () => {
    ...
    before('setup mock reducers and states', () => {
      ...
      incrementedState = mockUndoableReducer(mockInitialState, {
        type: 'INCREMENT',
      })
    ...
    describe('Replace reducers on the fly', () => {
      ...
      it('should use replaced reducer for new actions', () => {
        ...
        let expectedResult = tenfoldReducer(store.getState().present, {
          type: 'INCREMENT',
        })
        ...
        expectedResult = countReducer(store.getState().present, {
          type: 'INCREMENT',
        })
        store.dispatch({ type: 'INCREMENT' })
        expect(store.getState().present).to.equal(expectedResult)
    ...
    describe('Actions', () => {
      ...
      it('should not record non state changing actions', () => {
        let dummyState = mockUndoableReducer(incrementedState, {
          type: 'DUMMY',
        })
      ...
      it('should synchronize latest unfiltered state to present when filtering actions', () => {
        if (testConfig && testConfig.excludedActions) {
          ...
          let unsynchronized = mockUndoableReducer(
            mockInitialState,
            excludedAction,
          )
          let synchronized = synchronizedFilteredReducer(
            mockInitialState,
            excludedAction,
          )
          expect(unsynchronized.present).to.deep.equal(synchronized.present)
          expect(unsynchronized._latestUnfiltered).to.not.deep.equal(
            synchronized._latestUnfiltered,
          )
          expect(synchronized.present).to.deep.equal(
            synchronized._latestUnfiltered,
          )
        }
      })
    ...
    describe('Undo', () => {
      ...
      it("should change present state to last element of 'past'", () => {
        if (undoableConfig && undoableConfig.limit >= 0) {
          expect(undoState.present).to.equal(
      ...
      it("should decrease length of 'past' by one", () => {
        if (undoableConfig && undoableConfig.limit >= 0) {
          expect(undoState.past.length).to.equal(
      ...
      it("should increase length of 'future' by one", () => {
        if (undoableConfig && undoableConfig.limit >= 0) {
          expect(undoState.future.length).to.equal(
      ...
      it("should do nothing if 'past' is empty", () => {
        let undoInitialState = mockUndoableReducer(
          mockInitialState,
          ActionCreators.undo(),
        )
        if (!mockInitialState.past.length) {
          expect(undoInitialState.present).to.deep.equal(
            mockInitialState.present,
          )
        }
      })
    ...
    describe('Redo', () => {
      ...
      it("should add a new element to 'past' from last state", () => {
        if (undoableConfig && undoableConfig.limit >= 0) {
          expect(redoState.past[redoState.past.length - 1]).to.equal(
            undoState.present,
          )
        }
      })
      ...
      it("should do nothing if 'future' is empty", () => {
        let secondRedoState = mockUndoableReducer(
          redoState,
          ActionCreators.redo(),
        )
    ...
    describe('JumpToPast', () => {
      ...
      it('should increase the length of future if successful', () => {
        ...
        if (testConfig && !testConfig.includeActions) {
          if (incrementedState.past.length > jumpToPastIndex) {
            expect(jumpToPastState.future.length).to.be.above(
      ...
      it('should decrease the length of past if successful', () => {
        if (incrementedState.past.length > jumpToPastIndex) {
          expect(jumpToPastState.past.length).to.be.below(
    ...
    describe('JumpToFuture', () => {
      ...
      it('should increase the length of past if successful', () => {
        if (mockInitialState.future.length > jumpToFutureIndex) {
          expect(jumpToFutureState.past.length).to.be.above(
      ...
      it('should decrease the length of future if successful', () => {
        if (mockInitialState.future.length > jumpToFutureIndex) {
          expect(jumpToFutureState.future.length).to.be.below(
    ...
    describe('Jump', () => {
      ...
      before('perform a jump action', () => {
        let doubleIncrementedState = mockUndoableReducer(incrementedState, {
          type: 'INCREMENT',
        })
        jumpToPastState = mockUndoableReducer(
      ...
      it('should do nothing if steps is 0', () => {
        let jumpToCurrentState = mockUndoableReducer(
          mockInitialState,
    ...
    describe('running getSlices', () => {
      if (testConfig && testConfig.checkSlices) {
        ...
        it('should work normally after referencing slices', () => {
          expect(seventhState.normalState.present).to.equal(
